...
// Including all 49 questions with advanced features like split colors, rotations, reflections, arrows, etc.

console.log('🔄 Generating comprehensive pattern reasoning SVG files...');
    ...
    questions: [
      ...
      },
      {
        ...
        grid: [
          ...
          [
            { shape: "triangle", color: "blue", style: "filled", size: "big", rotation: 0, reflected: false },
            { shape: "square", color: "blue", style: "outline", size: "big", rotation: 0, reflected: false },
            null // Missing cell - should be star, filled, large
          ]
        ],
        options: [
          { id: "1", shape: "star", color: "blue", style: "filled", size: "big", rotation: 0, reflected: false, label: "Large filled star" },
          { id: "2", shape: "circle", color: "blue", style: "outline", size: "big", rotation: 0, reflected: false, label: "Large outline circle" },
          { id: "3", shape: "square", color: "blue", style: "filled", size: "big", rotation: 0, reflected: false, label: "Large filled square" },
      ...
      },
      {
        ...
        options: [
          ...
          { id: "4", shape: "heart", color: "red", rotation: 0, reflected: true, size: "big", direction: "right", label: "Large heart facing right" }
        ],
        correctAnswer: "1"
      },
      {
...
// Extended color map with all colors used in the questions
const colorMap = {
...
function generateShapeDescription(item) {
  ...
  if (item.strokeWidth) description.push(`stroke${item.strokeWidth}`);
...
function generateSVGTile(item, isQuestionMark = false, isSelected = false, size = 'large') {
  ...
  if (item) {
    content = generateShapeContent(item, tileSize);
...
function generateShapeContent(item, tileSize = 96) {
  ...
  if (item.position) {
    switch(item.position) {
      case 'top': centerY = -15; break;
      case 'bottom': centerY = 15; break;
      case 'left': centerX = -15; break;
      case 'right': centerX = 15; break;
    }
  }
...
function generateCircleContent(item, colors, size, centerX, centerY) {
  ...
  if (item.color === 'split' && item.topColor && item.bottomColor) {
    ...
    content = `<defs>
      <clipPath id="circleTopHalf${rotation}_${item.topColor}_${item.bottomColor}">
        <rect x="${-size}" y="${-size}" width="${size*2}" height="${size}" transform="rotate(${rotation} 0 0)" />
      </clipPath>
      <clipPath id="circleBottomHalf${rotation}_${item.topColor}_${item.bottomColor}">
        <rect x="${-size}" y="0" width="${size*2}" height="${size}" transform="rotate(${rotation} 0 0)" />
      </clipPath>
    </defs>
    <circle cx="${centerX}" cy="${centerY}" r="${size / 2}" fill="${topColors.fill}" stroke="${topColors.stroke}" stroke-width="2" clip-path="url(#circleTopHalf${rotation}_${item.topColor}_${item.bottomColor})" ${item.reflected ? 'transform="scale(-1, 1)"' : ''}/>
    <circle cx="${centerX}" cy="${centerY}" r="${size / 2}" fill="${bottomColors.fill}" stroke="${bottomColors.stroke}" stroke-width="2" clip-path="url(#circleBottomHalf${rotation}_${item.topColor}_${item.bottomColor})" ${item.reflected ? 'transform="scale(-1, 1)"' : ''}/>`;
    
    if (item.modifier === 'dot') {
      const dotX = centerX + (rotation === 90 ? size/4 : rotation === 180 ? 0 : rotation === 270 ? -size/4 : 0);
      const dotY = centerY + (rotation === 0 ? -size/4 : rotation === 90 ? 0 : rotation === 180 ? size/4 : 0);
      content += `<circle cx="${dotX}" cy="${dotY}" r="${size/8}" fill="black"/>`;
    }
    return content;
  }
  
  // Regular circle
  content = `<circle cx="${centerX}" cy="${centerY}" r="${size / 2}" fill="${item.style === 'outline' ? 'none' : colors.fill}" stroke="${colors.stroke}" stroke-width="2" ${item.reflected ? 'transform="scale(-1, 1)"' : ''}/>`;
...
function generateSquareContent(item, colors, size, centerX, centerY) {
  ...
  if (count > 1) {
    ...
    positions.forEach(pos => {
      content += `<rect x="${pos.x - size/6}" y="${pos.y - size/6}" width="${size/3}" height="${size/3}" fill="${item.style === 'outline' ? 'none' : colors.fill}" stroke="${colors.stroke}" stroke-width="2" rx="2"/>`;
    });
    return content;
  }
  ...
  if (item.color === 'split' && item.topColor && item.bottomColor) {
    ...
    content = `<defs>
      <clipPath id="squareTopHalf${rotation}_${item.topColor}_${item.bottomColor}">
        <rect x="${-size}" y="${-size}" width="${size*2}" height="${size}" transform="rotate(${rotation} 0 0)" />
      </clipPath>
      <clipPath id="squareBottomHalf${rotation}_${item.topColor}_${item.bottomColor}">
        <rect x="${-size}" y="0" width="${size*2}" height="${size}" transform="rotate(${rotation} 0 0)" />
      </clipPath>
    </defs>
    <rect x="${centerX - size/2}" y="${centerY - size/2}" width="${size}" height="${size}" fill="${topColors.fill}" stroke="${topColors.stroke}" stroke-width="2" rx="3" clip-path="url(#squareTopHalf${rotation}_${item.topColor}_${item.bottomColor})" ${item.reflected ? 'transform="scale(-1, 1)"' : ''}/>
    <rect x="${centerX - size/2}" y="${centerY - size/2}" width="${size}" height="${size}" fill="${bottomColors.fill}" stroke="${bottomColors.stroke}" stroke-width="2" rx="3" clip-path="url(#squareBottomHalf${rotation}_${item.topColor}_${item.bottomColor})" ${item.reflected ? 'transform="scale(-1, 1)"' : ''}/>`;
    return content;
  }
  ...
  // Handle stripe patterns
  if (item.modifier === 'stripe' || item.modifier === 'horizontal-stripe') {
    const patternId = `stripePatternSquare_${item.modifier || 'stripe'}_${Math.random().toString(36).substr(2, 9)}`;
    content = `<defs>
      <pattern id="${patternId}" patternUnits="userSpaceOnUse" width="4" height="4">
        <rect width="4" height="4" fill="${colors.fill}"/>
        ${item.modifier === 'horizontal-stripe' ? '<rect width="4" height="2" fill="black"/>' : '<rect width="2" height="4" fill="black"/>'}
      </pattern>
    </defs>
    <rect x="${centerX - size/2}" y="${centerY - size/2}" width="${size}" height="${size}" fill="url(#${patternId})" stroke="${customStroke}" stroke-width="2" rx="3" ${item.reflected ? 'transform="scale(-1, 1)"' : ''}/>`;
  } else {
    content = `<rect x="${centerX - size/2}" y="${centerY - size/2}" width="${size}" height="${size}" fill="${item.style === 'outline' ? 'none' : colors.fill}" stroke="${customStroke}" stroke-width="2" rx="3" ${item.reflected ? 'transform="scale(-1, 1)"' : ''}/>`;
  }
...
function generateTriangleContent(item, colors, size, centerX, centerY) {
  ...
  if (count > 1) {
    ...
    let content = '';
    positions.forEach(pos => {
      content += `<polygon points="${pos.x},${pos.y - size/6} ${pos.x + size/6},${pos.y + size/6} ${pos.x - size/6},${pos.y + size/6}" fill="${item.style === 'outline' ? 'none' : colors.fill}" stroke="${colors.stroke}" stroke-width="2"/>`;
    });
    return content;
  }
  ...
  if (item.color === 'split' && item.topColor && item.bottomColor) {
    ...
    return `<defs>
      <clipPath id="triangleTopHalf${rotation}_${item.topColor}_${item.bottomColor}">
        <rect x="${-size*1.5}" y="${-size}" width="${size*3}" height="${size}" transform="rotate(${rotation} 0 0)" />
      </clipPath>
      <clipPath id="triangleBottomHalf${rotation}_${item.topColor}_${item.bottomColor}">
        <rect x="${-size*1.5}" y="0" width="${size*3}" height="${size}" transform="rotate(${rotation} 0 0)" />
      </clipPath>
    </defs>
    <polygon points="${centerX},${centerY - size/2} ${centerX + size/2},${centerY + size/2} ${centerX - size/2},${centerY + size/2}" fill="${topColors.fill}" stroke="${topColors.stroke}" stroke-width="2" clip-path="url(#triangleTopHalf${rotation}_${item.topColor}_${item.bottomColor})" ${item.reflected ? 'transform="scale(-1, 1)"' : ''}/>
    <polygon points="${centerX},${centerY - size/2} ${centerX + size/2},${centerY + size/2} ${centerX - size/2},${centerY + size/2}" fill="${bottomColors.fill}" stroke="${bottomColors.stroke}" stroke-width="2" clip-path="url(#triangleBottomHalf${rotation}_${item.topColor}_${item.bottomColor})" ${item.reflected ? 'transform="scale(-1, 1)"' : ''}/>`;
  }
  
  // Handle stripe patterns
  let content = '';
  if (item.modifier === 'stripe') {
    const patternId = `stripePattern_${Math.random().toString(36).substr(2, 9)}`;
    content = `<defs>
      <pattern id="${patternId}" patternUnits="userSpaceOnUse" width="4" height="4">
        <rect width="4" height="4" fill="${colors.fill}"/>
    ...
    <polygon points="${centerX},${centerY - size/2} ${centerX + size/2},${centerY + size/2} ${centerX - size/2},${centerY + size/2}" fill="url(#${patternId})" stroke="${colors.stroke}" stroke-width="2" transform="rotate(${rotation} ${centerX} ${centerY}) ${item.reflected ? 'scale(-1, 1)' : ''}"/>`;
  } else {
    content = `<polygon points="${centerX},${centerY - size/2} ${centerX + size/2},${centerY + size/2} ${centerX - size/2},${centerY + size/2}" fill="${item.style === 'outline' ? 'none' : colors.fill}" stroke="${colors.stroke}" stroke-width="2" transform="rotate(${rotation} ${centerX} ${centerY}) ${item.reflected ? 'scale(-1, 1)' : ''}"/>`;
  }
...
function generateStarContent(item, colors, size, centerX, centerY) {
  ...
  if (starCount === 1 && item.color === 'split' && item.topColor && item.bottomColor) {
    ...
    return `<defs>
      <clipPath id="leftHalf${randId}">
        <rect x="${centerX - starSize * 1.5}" y="${centerY - starSize * 1.5}" width="${starSize * 1.5}" height="${starSize * 3}" transform="rotate(${rotation} ${centerX} ${centerY})" />
      </clipPath>
      <clipPath id="rightHalf${randId}">
        <rect x="${centerX}" y="${centerY - starSize * 1.5}" width="${starSize * 1.5}" height="${starSize * 3}" transform="rotate(${rotation} ${centerX} ${centerY})" />
      </clipPath>
    </defs>
    <polygon points="${starPoints}" fill="${topColors.fill}" stroke="${colors.stroke}" stroke-width="1" clip-path="url(#leftHalf${randId})"/>
    <polygon points="${starPoints}" fill="${bottomColors.fill}" stroke="${colors.stroke}" stroke-width="1" clip-path="url(#rightHalf${randId})"/>`;
  }
  
  // Multiple stars or single regular star
  ...
  for (let i = 0; i < starCount; i++) {
    ...
    content += `<polygon points="${starPoints}" fill="${colors.fill}" stroke="${colors.stroke}" stroke-width="1"/>`;
  }
...
function generateStarsContent(item, colors, size, centerX, centerY) {
  ...
  for (let i = 0; i < numStars; i++) {
    ...
    content += `<polygon points="${points.join(' ')}" fill="${colors.fill}" stroke="${colors.stroke}" stroke-width="1"/>`;
  }
...
function generateArrowContent(item, colors, size, centerX, centerY) {
  const arrowStrokeWidth = item.strokeWidth === 1 ? 0.5 : item.strokeWidth === 3 ? 5 : 2;
  const arrowSize = size * 1.2; // Make arrows 20% larger
  
  // Create arrow path pointing right, then rotate/reflect as needed
...
function generateAllSVGs() {
  ...
  Object.values(QUESTION_GROUPS).forEach(group => {
    ...
    group.questions.forEach(question => {
      ...
      if (question.sequence) {
        // Regular sequence or matrix with sequence array
        sequenceItems = question.sequence.filter(item => item !== null);
      } else if (question.grid) {
        // Matrix transformation with grid structure
        question.grid.forEach((row, rowIndex) => {
          row.forEach((item, colIndex) => {
            if (item !== null) {
              sequenceItems.push({
                ...item,
                gridPosition: `r${rowIndex + 1}c${colIndex + 1}`
              });
            }
          });
        });
      ...
      // Generate sequence/grid SVGs
      sequenceItems.forEach((item, index) => {
        const description = generateShapeDescription(item);
        let filename;
        
        if (item.gridPosition) {
          filename = `q_${String(question.id).padStart(2, '0')}_grid_${item.gridPosition}_${description}.svg`;
        } else {
          filename = `q_${String(question.id).padStart(2, '0')}_sequence_${index + 1}_${description}.svg`;
        }
      ...
      fileManifest.push({ 
        question: question.id, 
        type: 'question_mark', 
        filename: questionMarkFilename, 
        description: 'Question mark placeholder' 
      });
      totalFiles++;
    });
    
    console.log(`  ✅ Completed ${group.questions.length} questions\n`);
  });
  
  // Write file manifest
  fs.writeFileSync(path.join(outputDir, 'manifest.json'), JSON.stringify(fileManifest, null, 2));
  
  console.log(`\n🎉 Successfully generated ${totalFiles} SVG files!`);
  ...
  const breakdown = {
    sequence: fileManifest.filter(f => f.type === 'sequence').length,
    options: fileManifest.filter(f => f.type === 'option').length,
    questionMarks: fileManifest.filter(f => f.type === 'question_mark').length
  };
  
  console.log(`  • Sequence items: ${breakdown.sequence}`);
  console.log(`  • Option items: ${breakdown.options}`);
  console.log(`  • Question marks: ${breakdown.questionMarks}`);
  console.log(`  • Total: ${totalFiles}`);
  
  return { totalFiles, breakdown, outputDir: path.resolve(outputDir) };
}

// Run the generation
try {
  const result = generateAllSVGs();
  
  console.log(`\n✨ All SVG files are ready for download from GitHub!`);
  console.log(`📦 Files can be found in: ${result.outputDir}`);
