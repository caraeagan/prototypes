...
export default function PatternTestEntryNew() {
  ...
  const handleStart = async () => {
    ...
    // Redirect to the pattern reasoning test with NEW flag
    router.push(`/student/${sessionId}/pattern-reasoning?filter=12plus&new=true`)
  }
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-xl p-8 max-w-md w-full">
        <div className="text-center mb-8">
          ...
          <h1 className="text-3xl font-bold text-gray-900 mb-2">Visual Pattern Test (NEW)</h1>
          <p className="text-gray-600">Test with guaranteed CSV download functionality</p>
        </div>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-blue-50 rounded-lg p-4">
            ...
            <ul className="text-sm text-blue-800 space-y-1">
              ...
              <li>• CSV download guaranteed at completion</li>
            </ul>
          </div>
          ...
          >
            {isStarting ? (
              ...
              'Start NEW Pattern Test'
            )}
          </button>
        </div>

        <div className="mt-6 text-center text-xs text-gray-500">
          <p>This version includes the CSV download fix</p>
        </div>
      </div>
    </div>
  )
